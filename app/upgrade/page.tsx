    ...
    <main className="relative overflow-hidden min-h-screen flex flex-col">
      ...
      <section className="flex-1 flex flex-col items-center justify-center px-4 py-16">
        <div className="mb-10 text-center max-w-xl">
          <div className="w-12 h-12 mx-auto mb-5 rounded-full bg-gray-50 border border-gray-200 flex items-center justify-center">
        ...
        {!checked ? (
          <div className="w-full max-w-2xl grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[0, 1].map((i) => (
              <div key={i} className="h-[180px] rounded-2xl bg-brand-50/40 border border-gray-200 animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="w-full max-w-2xl grid grid-cols-1 sm:grid-cols-2 gap-4">
            {PAID_TIERS.map((t) => (
              <button
                ...
                className={`text-left rounded-2xl border p-6 transition-all cursor-pointer ${
                  t.badge === "Best Value"
                    ? "bg-brand-50 border-brand-400 hover:border-brand-400"
